...
data "docker_registry_image" "docker_image" {
  name = "cf3005/ctdc-base:${data.coder_parameter.docker_os.value}-${local.software[join("-",data.coder_parameter.select_ide.value)].tag}"
}
...
data "coder_parameter" "select_ide" {
  ...
  option {
    ...
    value = "vsc"
    icon = "https://cdn.jsdelivr.net/gh/walkxcode/dashboard-icons/png/vsc.png"
  ...
  validation {
    error = "Invalid IDE ! You cannot select both none and another IDE !"
    regex = "(none)|(vsc)|(fleet)"
  }
...
resource "coder_agent" "main" {
  ...
  os             = "linux"
  startup_script = local.software[join("-",data.coder_parameter.select_ide.value)].startup_script
...
resource "coder_app" "code-server" {
  count = local.software[join("-",data.coder_parameter.select_ide.value)].count_vsc
...
resource "coder_app" "fleet" {
  count = local.software[join("-",data.coder_parameter.select_ide.value)].count_fleet
  agent_id = coder_agent.main.id
  slug     = "fleet"
...
resource "coder_metadata" "workspace_info" {
  ...
  item {
    key = "IDE"
    value = local.software[join("-",data.coder_parameter.select_ide.value)].name
  }
}